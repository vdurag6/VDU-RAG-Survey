...
VENUES = [
 ("Vision", ["CVPR", "ICCV", "ECCV", "WACV", "ACM MM"]),
 ("Language", ["ACL", "EMNLP", "NAACL", "COLING", "TACL",
               "Computational Linguistics"]),
 ("Information retrieval", ["SIGIR", "CIKM", "KDD", "RecSys",
                            "The Web Conference", "ACM TOIS", "IP&M"]),
 ("Machine learning", ["NeurIPS", "ICLR", "AAAI", "IJCAI", "JMLR", "AIJ"]),
 ("Document analysis", ["ICDAR", "DAS", "IJDAR"]),
 ("Vision journals", ["IEEE TPAMI", "IJCV", "IEEE TIP", "IEEE TMM", "CVIU",
                      "Pattern Recognition"]),
 ("Applied and general", ["IEEE TNNLS", "Neural Networks", "KBS",
                          "Information Sciences", "ACM TIST", "ESWA",
                          "IEEE Access"]),
]
...
listed = set()
vt = ["| Venue | Type | Field | Papers | Add |", "|---|:-:|:-:|:-:|:-:|"]
for field, vs in VENUES:
    for v in vs:
        listed.add(v)
        n = seen.get(v, 0)
        kind = "journal" if v in JOURNALS else "conference"
        mark = f"**{n}**" if n else "—"
        vt.append(f"| {v} | {kind} | {field} | {mark} | "
                  f"[➕](../../issues/new?template=add-paper.yml) |")
other = sum(n for k, n in seen.items() if k not in listed and k != 'preprint')
...
print(f"rebuilt: {n} papers, {recent} from 2025-26, {sum(len(v) for _,v in VENUES)} venues listed")
